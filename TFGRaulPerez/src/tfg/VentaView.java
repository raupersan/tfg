...
public class VentaView {
	...
	public static ScrollPane crearVistaVentas(String dbUrl) {
		...
		clienteField.textProperty().addListener((obs, oldText, newText) -> {
			...
			try (Connection conn = DriverManager.getConnection(dbUrl);
					PreparedStatement stmt = conn.prepareStatement(
							"SELECT nombre FROM clientes WHERE LOWER(nombre) LIKE ? ORDER BY nombre ASC")) {
		...
		productoField.textProperty().addListener((obs, oldText, newText) -> {
			...
			try (Connection conn = DriverManager.getConnection(dbUrl);
					PreparedStatement stmt = conn.prepareStatement(
							"SELECT nombre FROM productos WHERE LOWER(nombre) LIKE ? ORDER BY nombre ASC")) {
		...
		cargarVentasDesdeBD(dbUrl, listaVentas);
		...
		layout.getChildren().addAll(clienteField, productoField, cantidadField, botonesVenta,
				new Label("Buscar ventas:"), botonesBusqueda, botonesExportar, new Label("Historial de Ventas:"),
				tablaVentas);
	...
	private static void cargarVentasDesdeBD(String dbUrl, ObservableList<String> listaVentas) {
		listaVentas.clear();
		try (Connection conn = DriverManager.getConnection(dbUrl);
				Statement stmt = conn.createStatement();
				ResultSet rs = stmt.executeQuery("SELECT * FROM ventas ORDER BY fecha DESC")) {
